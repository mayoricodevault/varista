'use strict';
xively.controller('coffeemenuController', ['$scope', 'Socket','$location','$anchorScroll', 'localStorageService',function ($scope,Socket, $location, $anchorScroll,localStorageService) {
    Socket.connect();
    ...
    var idCooffeeLocal=localStorageService.get('idCooffeeMenu');
    $scope.idCooffeeMenu=idCooffeeLocal || 0;
    $scope.$watch('idCooffeeMenu',function () {
        localStorageService.set('idCooffeeMenu',$scope.idCooffeeMenu);   
    },true);
    
    
    
    var people2a=localStorageService.get('people2');
    $scope.people2=people2a || [];
    $scope.$watch('people2',function () {
        localStorageService.set('people2',$scope.people2);   
    },true);
    ...
    $scope.addPerson = function() {
        $scope.idCooffeeMenu++;
    $scope.people2.push({id:$scope.idCooffeeMenu,name:"Paul Landaeta "+$scope.idCooffeeMenu,coffe:"Ice Coffee2"});
  //   $location.hash('person' + ($scope.j));
   //$anchorScroll();
    };
}]);